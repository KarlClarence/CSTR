...
# 尝试不同的初始温度猜测值，通常CSTR有多个稳态点，温度是区分它们的关键
initial_T_guesses = np.linspace(300, 450, 10)  # 从 300K 到 450K 尝试 10 个初始温度猜测
found_steady_states = []

for i, initial_T_guess in enumerate(initial_T_guesses):
    initial_guess_cA = 3.0  # 浓度可以保持不变，主要改变温度
    ...
    try:
        ...
        # 检查是否是新的、不重复的稳态点（考虑浮点数精度）
        is_new_state = True
        for s_state in found_steady_states:
            if np.allclose(s_state, current_steady_state, atol=1e-3):  # 容忍度 0.001
                is_new_state = False
                break
...
# 从找到的稳定点中选择一个用于协方差传播 (这里默认选择第一个，您可以根据稳定性分析结果手动修改)
# 理想情况下，您会选择一个被报告为“稳定”的稳态点
if found_steady_states:
    # 过滤出稳定的操作点
    ...
    if stable_operating_points:
        ...
        print("\n警告: 未找到任何稳定的稳态点。协方差传播可能仍会溢出。")
        nominal_state = found_steady_states[0]  # 如果没有稳定点，则使用第一个找到的（可能是）不稳定的点

else:
    print("\n错误: 未找到任何稳态点，无法进行协方差传播。请检查您的模型和初始猜测。")
    exit()  # 如果没有稳态点，就退出程序

nominal_inputs = ss_inputs  # 稳态输入不变
...
for k in range(num_steps):
    P_k = P_history[-1]
    # 协方差传播公式: P_{k+1} = A_d P_k A_d^T + Q_k
...
def plot_covariance_ellipse(ax, mean, cov, confidence_level=0.95, **kwargs):
    ...
    if not (np.all(np.isfinite(cov)) and cov[0, 0] > 0 and cov[1, 1] > 0 and np.linalg.det(cov) > 0):
        # print(f"警告: 协方差矩阵在均值 {mean} 处不是正定或有效，无法绘制椭圆。协方差: \n{cov}")
        return None
